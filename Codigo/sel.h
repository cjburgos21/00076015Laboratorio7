...
Matrix createLocalK(int e,mesh &m){
    ...
    Matrix matrixT,matrixK,matrixL,matrixD, matrixE, matrixAe;
    ...
    tao = m.getParameter(TAO_CONSTANTE);
    productRealMatrix(tao*J/(24*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,Beta,2,2,6),6,2,6),matrixT);
    kappa = m.getParameter(KAPPA_CONSTANTE);
    ...
    delta = m.getParameter(DELTA_CONSTANTE);
    ...
    lambda = m.getParameter(LAMBDA_CONSTANTE);
    ...
    zeroes(K,9);
    ubicarSubMatriz(K,0,5,0,5,sumMatrix(matrixT,matrixK,6,6));
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixE,matrixAe,6,3));
...
Vector createLocalb(int e,mesh &m){
    ...
    float f_x = m.getParameter(PSIX_CONSTANTE);
    float f_y = m.getParameter(PSIY_CONSTANTE);
    float eta = m.getParameter(ETA_CONSTANTE);
